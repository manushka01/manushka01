# Hi 👋, I'm Anushka Prajapati

### 💻 Full Stack Developer | CSE Graduate

I'm a Computer Science & Engineering graduate passionate about **full stack development, problem-solving, and building modern web applications**. I enjoy working across both frontend and backend to create complete, responsive, and user-friendly applications.

---

## 👨‍💻 About Me

* 🎓 Computer Science & Engineering Graduate
* 💻 Full Stack Developer
* 🚀 Passionate about building real-world web applications
* 🧩 Interested in problem-solving and software development
* 🌱 Always learning and exploring new technologies
* 🤝 Open to collaboration and new opportunities

---

## 🛠️ Tech Stack

### Frontend

HTML • CSS • JavaScript • React • TypeScript • Tailwind CSS

### Backend

Node.js • Express.js • REST APIs

### Database

MongoDB • MySQL • Firebase

### Programming

C • C++ • JavaScript • TypeScript

### Tools & Platforms

Git • GitHub • VS Code • Vite • Postman • MongoDB Atlas

---

## 🚀 Featured Projects

### 🌱 KrishiMitra AI

An AI-powered farming intelligence platform designed to provide farmers with useful insights, recommendations, and assistance.

**Tech:** JavaScript • Node.js • Express.js • AI APIs

---

### 🍴 Food Recipe Finder

A responsive web application that allows users to discover and explore different recipes through an interactive interface.

**Tech:** HTML • CSS • JavaScript

---

### 🧮 Scientific Calculator

A modern calculator application supporting scientific mathematical operations with a responsive user interface.

**Tech:** React • JavaScript

---

### 💼 Portfolio Website

A personal portfolio website showcasing my skills, projects, and development journey.

**Tech:** HTML • CSS • JavaScript

---

## 📚 Currently Learning

* Advanced JavaScript & TypeScript
* React.js
* Node.js & Express.js
* MongoDB & Database Management
* REST API Development
* Cloud & Deployment
* Data Structures & Algorithms

---

## 🎯 My Goals

* Build scalable and efficient web applications
* Strengthen my full stack development skills
* Improve problem-solving and DSA
* Contribute to open-source projects
* Build impactful real-world software

---

## 🤝 Let's Connect

I'm always interested in learning, collaborating on projects, and connecting with other developers.

⭐ Feel free to explore my repositories and check out my projects!

---

### 🚀 Thanks for visiting my profile!
